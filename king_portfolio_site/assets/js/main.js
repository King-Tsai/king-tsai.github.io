(function () {
  ...
  function render() {
    ...
    (data.education || []).forEach((item) => {
      ...
      const awards = (item.awards || []).map((award) => `
        <div class="education-award">
          ${award.image ? `<img src="${escapeHtml(award.image)}" alt="${escapeHtml(award.title)}">` : ''}
          <div>${escapeHtml(award.title)}</div>
        </div>`).join('');
      card.innerHTML = `
        ...
        ${awards ? `<div class="education-awards">${awards}</div>` : ''}`;
